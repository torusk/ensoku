...
EnsokuはSuiネットワーク上に構築された分散型アプリケーション（dApp）で、ユーザーが遠足スポットでの存在を証明し、提案を送信できるようにします。
...
- **`contract/`**: Moveスマートコントラクトが含まれています。
  - `sources/ensoku.move`: 位置チェックインと提案のためのメインモジュール。
...
- **遠足チェックイン**: オンチェーンで遠足スポットへの到着を証明します。
